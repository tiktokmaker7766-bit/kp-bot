...
function validateUAEPhone(input) {
  ...
  if (!cleanPhone.startsWith('+971')) {
    return { valid: false, error: '⚠️ Номер телефона должен начинаться с кода ОАЭ: **+971**' };
  }
...
const estimatorScene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    ...
    await ctx.reply(
      ...
      'Введите номер референса проекта (только цифры):',
      { parse_mode: 'Markdown' }
    );
  ...
  async (ctx) => {
    ...
    if (ctx.callbackQuery) {
      ...
      if (data === 'client_individual') {
        ...
        await ctx.reply('👤 Введите **Имя и Фамилию** клиента (каждое слово с заглавной буквы):');
        return;
      }
      ...
      if (data === 'skip_trn' && state.stepClient === 'comp_trn') {
        ...
        await ctx.reply('👤 Введите **Имя и Фамилию** контактного лица (каждое слово с заглавной буквы):');
        return;
      }
    }
    ...
    if (ctx.message && ctx.message.text) {
      ...
      if (state.stepClient === 'ind_name') {
        ...
        await ctx.reply('📞 Введите номер телефона клиента (Обязательно начиная с **+971**):', { parse_mode: 'Markdown' });
        return;
      }
      ...
      if (state.stepClient === 'comp_name') {
        ...
        await ctx.reply(
          '📑 Введите 15-значный **TRN / Налоговый код** компании (например: *100000000000000*) или нажмите кнопку ниже:',
          {
            parse_mode: 'Markdown',
      ...
      if (state.stepClient === 'comp_contact') {
        ...
        await ctx.reply('📞 Введите номер телефона контактного лица (Обязательно начиная с **+971**):', { parse_mode: 'Markdown' });
        return;
      }
  ...
  async (ctx) => {
    ...
    if (ctx.message && ctx.message.text) {
      ...
      if (state.costSubStep === 'labor_same_rate') {
        ...
        await ctx.reply('📈 Введите **процент компании** на внутренние расходы:');
        return;
      }
